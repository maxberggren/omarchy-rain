...
// Omarchy's lock view with the blurred wallpaper replaced by live rain on
// the wallpaper. Everything about the password field is unchanged.
...
Item {
  ...
  Rectangle {
    ...
    RainView {
      id: rain
      anchors.fill: parent
      imageSource: root.loadBackground && root.backgroundPath ? ("file://" + String(root.backgroundPath).split("/").map(encodeURIComponent).join("/")) : ""
      cfg: root.rainCfg
      time: root.rainTime
